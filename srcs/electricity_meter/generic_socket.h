...
public :
   ...
   int get_sock() {return sock;};
	...
	//data transmittion
	bool send (const std::vector<byte> ) throw (socket_exception);
    int recv ( std::vector<byte> &received_bytes_buffer, int bytes_to_receive_count) throw (socket_exception);
